...
For Netlify deployment, do not commit `.env`.
Instead add the same variables in Netlify site settings under "Build & deploy > Environment > Environment variables".

After adding the Netlify environment variables, redeploy the site so the build picks up the values.

If the dashboard still shows `LocalStorage Fallback Client`, then one or more Firebase variables are still missing or invalid.
