...
app.get("/api/blogs", async (_req, res) => {
  try {
    const docs = await Blog.find().sort({ date: -1, createdAt: -1 });
...
app.put("/api/blogs/:id", async (req, res) => {
  try {
    const payload = { ...req.body };
    delete payload.id;
    const doc = await Blog.findByIdAndUpdate(req.params.id, payload, {
      new: true,
      runValidators: true,
    });
...
app.post("/api/gallery", async (req, res) => {
  try {
    const payload = { ...req.body };
    delete payload.id;
    const doc = await GalleryItem.create(payload);
    res.status(201).json(toPublicDoc(doc));
  } catch (error) {
    res.status(400).json({ message: error.message });
  }
...
app.put("/api/gallery/:id", async (req, res) => {
  try {
    const payload = { ...req.body };
    delete payload.id;
    const doc = await GalleryItem.findByIdAndUpdate(req.params.id, payload, {
      new: true,
      runValidators: true,
    });
    if (!doc)
...
app.put("/api/projects/:id", async (req, res) => {
  try {
    const payload = { ...req.body };
    delete payload.id;
    const doc = await Project.findByIdAndUpdate(req.params.id, payload, {
      new: true,
      runValidators: true,
    });
