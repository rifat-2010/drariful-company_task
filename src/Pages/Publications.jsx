  ...
  const clearFilters = () => {
    ...
    setFilteredPublications(
      [...publications].sort((a, b) =>
        sortOrder === "desc"
          ? getSortYear(b.year) - getSortYear(a.year)
          : getSortYear(a.year) - getSortYear(b.year),
      ),
    );
  };
      ...
      <section id="publications" className="py-12 bg-gray-50">
        <div className="container mx-auto px-4">
          <div className="max-w-6xl mx-auto">
            <div>
              <h2 className="text-3xl font-bold mb-2 text-center heading">
                Research & Publications
              </h2>
              <p className="primary-text text-center mb-8">
                I am an enthusiastic researcher with 44 publications so far (37
                published and 7 submitted) focusing on Nephropathology,
                Gynaepathology, Lymphoreticular pathology, Head neck pathology
                and molecular pathology. My carrier focus is translational
                cancer research, precision cancer medicine and clinical trial.
              </p>
            </div>

            {/* Search and Filter Bar */}
            <div className="bg-white p-5 rounded-xl shadow-md mb-8">
              <p className="primary-text text-center mb-5 font-semibold sub-title">
                Browse through my published research papers and ongoing studies
              </p>
              ...
              {/* Field Filters */}
              <div className="mb-6">
                <h3 className="text-sm font-medium sub-title mb-2">
                  Filter by Field
                </h3>
                  ...
                  {fieldOptions.map((option) => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-full text-sm font-medium transition-colors ${
                        filters.field.includes(option.value)
                          ? "bg-blue-50 heading"
                  ...
                  <div className="flex flex-wrap gap-2">
                    {authorshipOptions.map((option) => (
                      <button
                        key={option.value}
                        onClick={() =>
                          handleAuthorshipFilterChange(option.value)
                        }
                        className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${
                          filters.authorship.includes(option.value)
            ...
            {filteredPublications.length === 0 && (
              <div className="text-center py-16 bg-white rounded-lg shadow-sm">
                ...
                <button onClick={clearFilters} className="px-6 py-2 active-btn">
                  Clear Filters
                </button>
              </div>
            )}
