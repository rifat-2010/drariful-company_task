  ...
  const applyFilters = (term, category) => {
    ...
    if (term) {
      ...
      result = result.filter(blog =>
        ...
        (blog.category && blog.category.toLowerCase().includes(lowerCaseTerm))
      );
    }
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredBlogs.map(blog => (
                <article key={blog.id} className="bg-white rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-all flex flex-col group border border-gray-100">
                  <div className="relative h-48 overflow-hidden bg-gray-200">
                    {blog.coverImage ? (
                      <img
                        ...
                        onError={(e) => {
                          e.target.src = 'https://placehold.co/600x400/003878/FFF?text=Medical+Insight';
                        }}
                      />
                    ) : (
            ...
            <div className="text-center py-16 bg-white rounded-xl shadow-md border border-gray-100">
              <p className="primary-text text-lg mb-4">No articles found matching your criteria</p>
