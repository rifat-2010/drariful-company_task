...
const FeaturedProjects = () => {
  ...
  const getProjectImage = (project) => {
    ...
    if (project.field === 'Cancer' || project.field === 'Molecular Pathology') return image1;
    return image2;
  };
  ...
  return (
    <section id="projects" className="mt-8 lg:mt-16 bg-gray-50 py-12">
      <div className="w-11/12 mx-auto">
        ...
        <p className="mb-8 text-sm md:text-base text-center primary-text">Highlighted research initiatives and ongoing studies</p>
                ...
                <div>
                  ...
                  <div className="p-6">
                    <div className="flex justify-between items-start gap-2 mb-3">
                      <h3 className="text-base font-bold sub-title line-clamp-2" title={project.title}>{project.title}</h3>
                      <span className={`inline-block px-2.5 py-1 text-xs font-semibold rounded-full shrink-0 ${
                    ...
                    <p className="primary-text mb-4 line-clamp-3 text-justify text-sm leading-relaxed">{project.summary}</p>
                  </div>
                </div>
                  ...
                  <div className="flex items-center primary-text text-xs mb-4 font-bold">
                    ...
                    {project.collaborators && project.collaborators.length > 0 && (
                      <span className="flex items-center">
                        <FaUsers className="mr-1 text-blue-600" /> {project.collaborators.length} collaborator{project.collaborators.length !== 1 ? 's' : ''}
                      </span>
                    )}
                  </div>
              ...
              <div className="p-8 overflow-y-auto h-[calc(520px-14rem)] flex flex-col justify-between">
                <div>
                  <div className="flex justify-between items-start gap-4 mb-4">
                    <h3 className="text-lg md:text-xl font-extrabold text-gray-900 leading-tight">{activeProject.title}</h3>
                    <span className={`inline-block px-3 py-1 text-sm font-semibold rounded-full shrink-0 ${
                      activeProject.status === 'Published' || activeProject.status === 'Completed'
                        ? 'bg-blue-100 text-[#003878]'
                        : 'bg-orange-100 text-[#e88d67]'
                    }`}>
                  ...
                  <div className="flex items-center text-gray-500 text-xs font-bold mb-6 gap-3">
                    <span className="bg-blue-50 text-[#003878] px-2.5 py-1 rounded-full border border-blue-100">Field: {activeProject.field}</span>
                    <span className="bg-gray-100 px-2.5 py-1 rounded-full">Duration: {activeProject.year}</span>
                  </div>

                  <p className="text-gray-600 mb-6 text-justify text-sm leading-relaxed">{activeProject.description}</p>

                  {activeProject.collaborators && activeProject.collaborators.length > 0 && (
                    <div className="mb-6">
                      <h4 className="text-xs font-bold mb-2 text-gray-800 uppercase tracking-wider">Collaborators</h4>
